    ...
    protected BufferedImage apply(Image image) {
        for (int y = 0; y < image.bufferedImage().getHeight(); y++) {
            for (int x = 0; x < image.bufferedImage().getWidth(); x++) {
                ...
                int newRed = ColorUtils.findClosestColor(oldRed, kv[0]);
                int newGreen = ColorUtils.findClosestColor(oldGreen, kv[1]);
                int newBlue = ColorUtils.findClosestColor(oldBlue, kv[2]);
